...
export async function updateOrderById(req: AuthRequest, res: Response) {
  try {
    ...
    if (!order) return res.status(404).json({ error: 'Order not found.' });
      ...
      const updatedOrder = await Orders.findByIdAndUpdate(order._id, { $set: data }, { new: true });
...
export async function findOrdersByRestaurantId(req: AuthRequest, res: Response) {
  try {
    const user = req.user;
    if (!user) return res.status(401).send({ message: 'Unauthorized.' });

    const orders = await Orders.find({ restaurantId: user.employeeInformation.restaurantId });
...
export async function findOrdersByOrderType(req: AuthRequest, res: Response) {
  try {
    ...
    const orders = await Orders.find({ orderType, restaurantId: user.employeeInformation.restaurantId });
    res.status(200).json(orders);
  } catch (error) {
    console.error(error);
